...
clf = SVC(kernel='rbf', class_weight='balanced', probability=True)  # Enable probability for score plotting
...
for i in range(0, len(y)):
    ...
    y_prob = clf.predict_proba(query_X.reshape(1, -1))[:, 1]  # Probability for class 1
    
    # Get results and store scores
    if y_pred == 1:
        num_correct += 1
        genuine_scores.append(y_prob[0])  # Genuine match score
    else:
        num_incorrect += 1
        impostor_scores.append(y_prob[0])  # Impostor match score
