...
def test_datasets(X, y, dataset_name):
    ...
    euclidean_results = test_classifiers(euclidean_features, y, dataset_name)
    all_results["Euclidean Distance"] = euclidean_results

    print("\n--- Manhattan Distance Features ---")
    manhattan_features = compute_manhattan_features(X)
    manhattan_results = test_classifiers(manhattan_features, y, dataset_name)
    all_results["Manhattan Distance"] = manhattan_results

    print("\n--- Angles Between Landmarks ---")
    angle_features = compute_angles(X)
    angle_results = test_classifiers(angle_features, y, dataset_name)
    all_results["Angles Between Landmarks"] = angle_results

    print("\n--- Distance Ratios ---")
    ratio_features = compute_distance_ratios(X)
    ratio_results = test_classifiers(ratio_features, y, dataset_name)
    all_results["Distance Ratios"] = ratio_results

    print("\n--- Landmark Displacement Features ---")
    displacement_features = compute_landmark_displacement(X)
    displacement_results = test_classifiers(displacement_features, y, dataset_name)
    all_results["Landmark Displacement"] = displacement_results

    print("\n--- Landmark Density Features ---")
    density_features = compute_landmark_density(X)
    density_results = test_classifiers(density_features, y, dataset_name)
    all_results["Landmark Density"] = density_results

    print("\n--- Landmark Histograms ---")
    histogram_features = compute_landmark_histograms(X)
    histogram_results = test_classifiers(histogram_features, y, dataset_name)
    all_results["Landmark Histograms"] = histogram_results

    return all_results

# Function to highlight results with accuracy >= 0.8
def highlight_results(all_results, dataset_name):
    print(f"\n--- Highlighting Results for {dataset_name} with Accuracy >= 0.8 ---")
    for feature, results in all_results.items():
        for classifier, (accuracy, std) in results.items():
            if accuracy >= 0.8:
                print(f"{feature} - {classifier}: Accuracy = {accuracy:.2f} (+/- {std:.2f})")
...
# Test SoF dataset
sof_results = test_datasets(X_sof, y_sof, "SoF")
highlight_results(sof_results, "SoF")

# Test Caltech dataset
caltech_results = test_datasets(X_caltech, y_caltech, "Caltech")
highlight_results(caltech_results, "Caltech")